...
Install the required Python dependies:
```bash
...
- **Test Accuracy**: ~73.94% (average across folds)
...
**Average Test Accuracy**: **73.94%**

---

### **Observations**
1. **Best Fold**: Fold 4 achieved the highest test accuracy of **79.80%**, indicating strong generalization for that split of the dataset.
2. **Consistency**: The model performed consistently across all folds, with test accuracies ranging from **64.65%** to **79.80%**.
3. **Improvement**: The learning rate scheduler and regularization techniques (dropout and weight decay) contributed to stable training and improved test accuracy.

---

## **Future Work**
- **Hyperparameter Tuning**: Experiment with different learning rates, batch sizes, and dropout rates to further improve performance.
- **Advanced Architectures**: Explore more advanced models like ResNet or EfficientNet for better feature extraction.
- **Data Augmentation**: Add more augmentation techniques to improve generalization.
- **Ensemble Learning**: Combine predictions from all 5 folds to create an ensemble model for potentially higher accuracy.
- **Transfer Learning**: Use pre-trained models to leverage knowledge from larger datasets.
